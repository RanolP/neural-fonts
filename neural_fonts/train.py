...
parser: argparse.ArgumentParser = argparse.ArgumentParser(description="Train")
parser.add_argument(
    "--experiment_dir",
    dest="experiment_dir",
    required=True,
...
parser.add_argument(
    "--L1_penalty", dest="L1_penalty", type=int, default=100, help="weight for L1 loss"
)
parser.add_argument(
    "--Lconst_penalty",
    dest="Lconst_penalty",
...
parser.add_argument(
    "--resume", dest="resume", type=int, default=1, help="resume from previous training"
)
parser.add_argument(
    "--freeze_encoder",
    dest="freeze_encoder",
...
parser.add_argument(
    "--no_val", dest="no_val", type=int, default=None, help="no validation set is given"
)
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.flip_labels:
            model.build_model(
                is_training=True, inst_norm=args.inst_norm, no_target_source=True
            )
        else:
