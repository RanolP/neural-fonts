...
InputHandle = namedtuple(
    "InputHandle", ["real_data", "embedding_ids", "no_target_data", "no_target_ids"]
)
EvalHandle = namedtuple(
...
class UNet:
    ...
    def generator(
        self, images, embeddings, embedding_ids, inst_norm, is_training, reuse=False
    ):
        ...
        local_embeddings = tf.reshape(
            local_embeddings, [self.batch_size, 1, 1, self.embedding_dim]
        )
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        summary_handle = SummaryHandle(
            d_merged=d_merged_summary, g_merged=g_merged_summary
        )
    ...
    def retrieve_generator_vars(self):
        ...
        generate_vars = [
            var for var in all_vars if "embedding" in var.name or "g_" in var.name
        ]
    ...
    def validate_model(self, val_iter, epoch, step):
        ...
        sample_img_path = os.path.join(
            model_sample_dir, "sample_%02d_%04d.png" % (epoch, step)
        )
    ...
    def infer(self, source_obj, embedding_ids, model_dir, save_dir, progress_file):
        ...
        for labels, codes, source_imgs in source_iter:
            ...
            for i in range(len(fake_imgs)):
                ...
                # Normalize image
                fake_imgs[i][:, :, 0] = Image.fromarray(np.array(en_img) / 127.5 - 1.0)
    ...
    def infer_compare(self, source_obj, embedding_ids, model_dir, save_dir, show_ssim):
        ...
        for labels, codes, source_imgs in source_iter:
            ...
            for i in range(len(fake_imgs)):
                ...
                if show_ssim:
                    mean, Ssim = ssim(
                        fake_imgs[i], real_imgs[i], full=True, multichannel=True
                    )
                    S[i] = Ssim
            ...
            if show_ssim:
                merged_pair = np.concatenate(
                    [merged_pair, merge(scale_back(S), [self.batch_size, 1])], axis=1
                )
    ...
    def train(
        ...
        d_optimizer = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(
            loss_handle.d_loss, var_list=d_vars
        )
        g_optimizer = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(
            loss_handle.g_loss, var_list=g_vars
        )
        tf.global_variables_initializer().run()
        ...
        data_provider = TrainDataProvider(
            self.data_dir, filter_by=fine_tune, no_val=no_val
        )
